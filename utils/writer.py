...
def img_writer_training(iou_loss, obj_loss, cls_loss, loss, lr, epoch, filename):
    ...
    fig.set_dpi(1240)
    ax_array = fig.subplots(1, 5, squeeze=False)
    ...
    # Plot for loss
    ax_array[0, 3].set_ylabel('Loss')
    ax_array[0, 3].plot(x, loss, marker = 'o')
    ax_array[0, 3].set_xlabel('Epoch')

    # Plot for learning rate
    ax_array[0, 4].set_ylabel('Learning rate')
    ax_array[0, 4].plot(x, lr, marker = 'o')
    # https://stackoverflow.com/questions/21393802/how-to-specify-values-on-y-axis-of-a-matplotlib-plot
    ax_array[0, 4].get_autoscaley_on()
    ax_array[0, 4].invert_yaxis()
    ax_array[0, 4].set_xlabel('Epoch')


    fig.savefig(filename+'_training_metrics.png')
    plt.close()

def img_writer_evaluation(precision, recall, mAP, f1, ap_cls, epoch, filename):
    ...
    # Plot for f1
    ax_array[0, 3].set_ylabel('F1')
    ax_array[0, 3].plot(x, f1, marker = 'o')
    ax_array[0, 3].set_xlabel('Epoch')
    ax_array[0, 3].set_ybound([0, 1])


    # Plot for ap_cls
    ax_array[0, 4].set_ylabel('AP CLS')
    ax_array[0, 4].plot(x, ap_cls, marker='o')
    ax_array[0, 4].set_xlabel('Epoch')
    ax_array[0, 4].set_ybound([0, 1])
